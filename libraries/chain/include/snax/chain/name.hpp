...
#define N(X) snax::chain::string_to_name(#X)
...
namespace std {
   template<> struct hash<snax::chain::name> : private hash<uint64_t> {
      typedef snax::chain::name argument_type;
...
namespace fc {
  class variant;
  void to_variant(const snax::chain::name& c, fc::variant& v);
  void from_variant(const fc::variant& v, snax::chain::name& check);
} // fc


FC_REFLECT( snax::chain::name, (value) )

...
namespace snax { namespace chain { namespace config {
...
const static uint64_t system_account_name    = N(snax);
...
const static uint32_t ram_market_open  = 1;
...
const static snax::chain::wasm_interface::vm_type default_wasm_runtime = snax::chain::wasm_interface::vm_type::wabt;
...
} } } // namespace snax::chain::config

constexpr uint64_t SNAX_PERCENT(uint64_t value, uint32_t percentage) {
   return (value * percentage) / snax::chain::config::percent_100;
}

template<typename Number>
Number SNAX_PERCENT_CEIL(Number value, uint32_t percentage) {
   return ((value * percentage) + snax::chain::config::percent_100 - snax::chain::config::percent_1)  / snax::chain::config::percent_100;
}

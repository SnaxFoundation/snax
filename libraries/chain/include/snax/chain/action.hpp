...
namespace snax { namespace chain {
    ...
    *  In the context of snax, every action is dispatched to the handler defined
   ...
   struct action {
      ...
      template<typename T>
      T data_as()const {
         SNAX_ASSERT( account == T::get_account(), action_type_exception, "account is not consistent with action struct" );
         SNAX_ASSERT( name == T::get_name(), action_type_exception, "action name is not consistent with action struct"  );
...
FC_REFLECT( snax::chain::permission_level, (actor)(permission) )
FC_REFLECT( snax::chain::action, (account)(name)(authorization)(data) )

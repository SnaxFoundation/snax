...
void validate_authority_precondition( const apply_context& context, const authority& auth ) {
   for(const auto& a : auth.accounts) {
      ...
      if( a.permission.permission == config::snax_code_name ) // virtual snax.code permission does not really exist but is allowed
         continue;

      try {
         context.control.get_authorization_manager().get_permission({a.permission.actor, a.permission.permission});
      } catch( const permission_query_exception& ) {
         SNAX_THROW( action_validate_exception,
 ...
 */
void apply_snax_newaccount(apply_context& context) {
   auto create = context.act.data_as<newaccount>();
   try {
   context.require_authorization(create.creator);
//   context.require_write_lock( config::snax_auth_scope );
   auto& authorization = context.control.get_mutable_authorization_manager();

   SNAX_ASSERT( validate(create.owner), action_validate_exception, "Invalid owner authority");
   SNAX_ASSERT( validate(create.active), action_validate_exception, "Invalid active authority");

   auto& db = context.db;

   auto name_str = name(create.name).to_string();

   SNAX_ASSERT( !create.name.empty(), action_validate_exception, "account name cannot be empty" );
   SNAX_ASSERT( name_str.size() <= 12, action_validate_exception, "account names can only be 12 chars long" );

   // Check if the creator is privileged
   const auto &creator = db.get<account_object, by_name>(create.creator);
   if( !creator.privileged ) {
      SNAX_ASSERT( name_str.find( "snax." ) != 0, action_validate_exception,
                  "only privileged accounts can have names that start with 'snax.'" );
   }

   auto existing_account = db.find<account_object, by_name>(create.name);
   SNAX_ASSERT(existing_account == nullptr, account_name_exists_exception,
...
void apply_snax_setcode(apply_context& context) {
   ...
   context.require_set_contract_ability(account);

   int64_t code_size = (int64_t)act.code.size();
   int64_t old_size  = (int64_t)account.code.size() * config::setcode_ram_bytes_multiplier;
   int64_t new_size  = code_size * config::setcode_ram_bytes_multiplier;

   SNAX_ASSERT( account.code_version != code_id, set_exact_code, "contract is already running this version of code" );
...
void apply_snax_setabi(apply_context& context) {
   ...
   context.require_set_abi_ability(account);
...
void apply_snax_updateauth(apply_context& context) {
   ...
   SNAX_ASSERT(!update.permission.empty(), action_validate_exception, "Cannot create authority with empty name");
   SNAX_ASSERT( update.permission.to_string().find( "snax." ) != 0, action_validate_exception,
               "Permission names that start with 'snax.' are reserved" );
   SNAX_ASSERT(update.permission != update.parent, action_validate_exception, "Cannot set an authority as its own parent");
   db.get<account_object, by_name>(update.account);
   SNAX_ASSERT(validate(update.auth), action_validate_exception,
              "Invalid authority: ${auth}", ("auth", update.auth));
   if( update.permission == config::active_name )
      SNAX_ASSERT(update.parent == config::owner_name, action_validate_exception, "Cannot change active authority's parent from owner", ("update.parent", update.parent) );
   if (update.permission == config::owner_name)
      SNAX_ASSERT(update.parent.empty(), action_validate_exception, "Cannot change owner authority's parent");
   else
      SNAX_ASSERT(!update.parent.empty(), action_validate_exception, "Only owner permission can have empty parent" );

   if( update.auth.waits.size() > 0 ) {
      auto max_delay = context.control.get_global_properties().configuration.max_transaction_delay;
      SNAX_ASSERT( update.auth.waits.back().wait_sec <= max_delay, action_validate_exception,
   ...
   if( permission ) {
      SNAX_ASSERT(parent_id == permission->parent, action_validate_exception,
...
void apply_snax_deleteauth(apply_context& context) {
//   context.require_write_lock( config::snax_auth_scope );
   ...
   SNAX_ASSERT(remove.permission != config::active_name, action_validate_exception, "Cannot delete active authority");
   SNAX_ASSERT(remove.permission != config::owner_name, action_validate_exception, "Cannot delete owner authority");
...
void apply_snax_linkauth(apply_context& context) {
//   context.require_write_lock( config::snax_auth_scope );

   auto requirement = context.act.data_as<linkauth>();
   try {
      SNAX_ASSERT(!requirement.requirement.empty(), action_validate_exception, "Required permission cannot be empty");
      ...
      SNAX_ASSERT(account != nullptr, account_query_exception,
      ...
      SNAX_ASSERT(code != nullptr, account_query_exception,
                 "Failed to retrieve code for account: ${account}", ("account", requirement.code));
      if( requirement.requirement != config::snax_any_name ) {
         const auto *permission = db.find<permission_object, by_name>(requirement.requirement);
         SNAX_ASSERT(permission != nullptr, permission_query_exception,
      ...
      if( link ) {
         SNAX_ASSERT(link->required_permission != requirement.requirement, action_validate_exception,
...
void apply_snax_unlinkauth(apply_context& context) {
//   context.require_write_lock( config::snax_auth_scope );
   ...
   auto link = db.find<permission_link_object, by_action_name>(link_key);
   SNAX_ASSERT(link != nullptr, action_validate_exception, "Attempting to unlink authority, but no link found");
   context.add_ram_usage(
      link->account,
      -(int64_t)(config::billable_size_v<permission_link_object>)
   );
...
void apply_snax_canceldelay(apply_context& context) {

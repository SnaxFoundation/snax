...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->add_flag("--add-code", add_code, localized("[code] add '${code}' permission to specified permission authority", ("code", name(config::snax_code_name))));
      permissions->add_flag("--remove-code", remove_code, localized("[code] remove '${code}' permission from specified permission authority", ("code", name(config::snax_code_name))));

      add_standard_transaction_options(permissions, "account@active");

      permissions->set_callback([this] {
         SNAXC_ASSERT( !(add_code && remove_code), "ERROR: Either --add-code or --remove-code can be set" );
         SNAXC_ASSERT( (add_code ^ remove_code) || !authority_json_or_file.empty(), "ERROR: authority should be specified unless add or remove code permission" );
         ...
         if ( need_parent || need_auth ) {
            ...
            if ( need_auth ) {
               ...
               auto code_name = name(config::snax_code_name);
   ...
   approve_producer_subcommand(CLI::App* actionRoot) {
      ...
      approve_producer->set_callback([this] {
            auto result = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so clisnax can still work with old buggy snaxnode versions
                               // Change to voter.value when clisnax no longer needs to support snaxnode versions older than 1.5.0
                               ("limit", 1)
            );
            auto res = result.as<snax::chain_apis::read_only::get_table_rows_result>();
            // Condition in if statement below can simply be res.rows.empty() when clisnax no longer needs to support snaxnode versions older than 1.5.0
   ...
   bidname_info_subcommand(CLI::App* actionRoot) {
      ...
      list_producers->set_callback([this] {
         auto rawResult = call(get_table_func, fc::mutable_variant_object("json", true)
                               ...
                               // Less than ideal upper_bound usage preserved so clisnax can still work with old buggy snaxnode versions
                               // Change to newname.value when clisnax no longer needs to support snaxnode versions older than 1.5.0
                               ("limit", 1));
         if ( print_json ) {
            std::cout << fc::json::to_pretty_string(rawResult) << std::endl;
            return;
...
//                                ->check(CLI::ExistingFile);
   ...
   auto set_code_callback = [&]() {
      ...
      if(!contract_clear){
        ...
        SNAX_ASSERT( !wasm.empty(), wasm_file_not_found, "no wasm file found ${f}", ("f", wasmPath) );
   ...
   auto stopKsnaxd = wallet->add_subcommand("stop", localized("Stop kxd (doesn't work with snaxnode)."), false);
   stopKsnaxd->set_callback([] {
   ...
   review->set_callback([&] {
      const auto result1 = call(get_table_func, fc::mutable_variant_object("json", true)
                                 ...
                                 // Less than ideal upper_bound usage preserved so clisnax can still work with old buggy snaxnode versions
                                 // Change to name(proposal_name).value when clisnax no longer needs to support snaxnode versions older than 1.5.0
                                 ("limit", 1)
                           );
      //std::cout << fc::json::to_pretty_string(result) << std::endl;

      const auto& rows1 = result1.get_object()["rows"].get_array();
      // Condition in if statement below can simply be rows.empty() when clisnax no longer needs to support snaxnode versions older than 1.5.0
      ...
      std::map<snax::account_name, std::pair<fc::time_point, vector<decltype(all_approvals)::iterator>>>  provided_approvers;
      ...
      if( show_approvals_in_multisig_review ) {
         ...
         if( new_multisig ) {
            for( auto& a : provided_approvers ) {
               ...
               if( rows4.empty() || rows4[0].get_object()["account"].as<snax::name>() != a.first ) {
                  continue;
               }
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, "snax.msig", "invalidate", variant_to_bin( N(snax.msig), "invalidate", args ) }});
   });

...
int main(int argc, char** argv)
{
   try {
      ...
      app().set_default_config_dir(root / "snax/snaxnode/config" );
      ...
      initialize_logging();
      ilog("snaxnode version ${ver}", ("ver", app().version_string()));
      ilog("snax root is ${root}", ("root", root.string()));
      ilog("snaxnode using configuration file ${c}", ("c", app().full_config_file_path().string()));
      ilog("snaxnode data directory is ${d}", ("d", app().data_dir().string()));
      app().startup();
      app().exec();

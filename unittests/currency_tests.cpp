...
class currency_tester : public TESTER {
   public:
      ...
      asset get_balance(const account_name& account) const {
         return get_currency_balance(N(snax.token), symbol(SY(4,CUR)), account);
      }
      ...
      auto issue(const account_name& to, const std::string& quantity, const std::string& memo = "") {
         auto trace = push_action(N(snax.token), N(issue), mutable_variant_object()
                                  ("to",       to)
                                  ("quantity", quantity)
                                  ("memo",     memo)
                                  );
      ...
      currency_tester()
      :TESTER(),abi_ser(json::from_string(snax_token_abi).as<abi_def>(), abi_serializer_max_time)
      {
         create_account( N(snax.token));
         set_code( N(snax.token), snax_token_wast );

         auto result = push_action(N(snax.token), N(create), mutable_variant_object()
                 ("issuer",       snax_token)
         ...
         result = push_action(N(snax.token), N(issue), mutable_variant_object()
                 ("to",       snax_token)
      ...
      static const std::string snax_token;
};

const std::string currency_tester::snax_token = name(N(snax.token)).to_string();
...
BOOST_AUTO_TEST_CASE( bootstrap ) try {
   ...
   currency_tester t;
   auto actual = t.get_currency_balance(N(snax.token), expected.get_symbol(), N(snax.token));
...
BOOST_FIXTURE_TEST_CASE( test_transfer, currency_tester ) try {
   ...
   {
      auto trace = push_action(N(snax.token), N(transfer), mutable_variant_object()
         ("from", snax_token)
         ("to",   "alice")
...
BOOST_FIXTURE_TEST_CASE( test_duplicate_transfer, currency_tester ) {
   ...
   BOOST_REQUIRE_THROW(push_action(N(snax.token), N(transfer), mutable_variant_object()
                                    ("from", snax_token)
                                    ("to",   "alice")
                                    ("quantity", "100.0000 CUR")
                                    ("memo", "fund Alice")),
...
BOOST_FIXTURE_TEST_CASE( test_overspend, currency_tester ) try {
   ...
   // Overspend!
   {
      ...
      BOOST_CHECK_EXCEPTION( push_action(N(alice), N(transfer), data),
                             snax_assert_message_exception, snax_assert_message_is("overdrawn balance") );
      produce_block();
...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      symbol sys(4, "SNAX");
      BOOST_REQUIRE_EQUAL(SY(4,SNAX), sys.value());
      BOOST_REQUIRE_EQUAL("4,SNAX", sys.to_string());
      BOOST_REQUIRE_EQUAL("SNAX", sys.name());
...
BOOST_FIXTURE_TEST_CASE( test_input_quantity, currency_tester ) try {
   ...
   {
      auto trace = transfer(snax_token, N(alice), "100.0000 CUR");
   ...
   {
      BOOST_REQUIRE_THROW(transfer(N(alice), N(carl), "20.50 USD"), snax_assert_message_exception);
   }

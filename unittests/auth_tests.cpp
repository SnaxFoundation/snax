...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for snax reqauth action with alice's spending key
   chain.link_authority("alice", "snax", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "snax", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with snax reqauth
   chain.unlink_authority("alice", "snax", "reqauth");
   ...
   // Link authority for any snax action with alice's scud key
   chain.link_authority("alice", "snax", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "snax", "first",  "reqauth");
...
try {
   ...
   // Creating account with snax. prefix with privileged account
   chain.create_account("snax.test1");

   // Creating account with snax. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("snax.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'snax.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "snax", "snax.any", "reqauth" );
   chain.link_authority( "bob", "snax", "snax.any", "reqauth" );

   /// this should succeed because snax::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = snax::chain::resource_limits::resource_usage_object;
   using by_owner = snax::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link snax::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };

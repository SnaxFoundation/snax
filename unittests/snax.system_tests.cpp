...
#include "snax_system_tester.hpp"

using namespace snax_system;

BOOST_AUTO_TEST_SUITE(snax_system_tests)
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, snax_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( producer_register_unregister, snax_system_tester ) try {
   ...
   auto key2 =  fc::crypto::public_key( std::string("SNAX5jnmSKrzdBHE9n8hw58y7yxFWBC8SNiG7m8S1crJH3KvAnf9o6") );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(alice1111111), N(regproducer), mvo()
                                               ("producer",  "alice1111111")
                                               ("producer_key", key2 )
                                               ("url", "http://block.two")
...
BOOST_FIXTURE_TEST_CASE( vote_for_two_producers, snax_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   //alice1111111 becomes a producer
   fc::variant params = producer_parameters_example(1);
...
BOOST_FIXTURE_TEST_CASE( proxy_register_unregister_keeps_stake, snax_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( proxy_stake_unstake_keeps_proxy_flag, snax_system_tester ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( proxy_actions_affect_producers, snax_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE(producer_pay, snax_system_tester, * boost::unit_test::tolerance(1e-10)) try {
   ...
   // produce enough blocks so new schedule kicks in and defproducera produces some blocks
   {
      ...
      const int64_t  initial_savings           = 0;
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, snax_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...

   {
      ...
      const asset    initial_vpay_balance      = get_balance(N(snax.vpay));
      ...
      const asset    vpay_balance      = get_balance(N(snax.vpay));
...

   {
      ...
      BOOST_REQUIRE_EQUAL( error("missing authority of snax"),
...
BOOST_FIXTURE_TEST_CASE(producers_upgrade_system_contract, snax_system_tester) try {
...
BOOST_FIXTURE_TEST_CASE( voters_actions_affect_proxy_and_producers, snax_system_tester, * boost::unit_test::tolerance(1e+6) ) try {
   cross_15_percent_threshold();
...
BOOST_FIXTURE_TEST_CASE( vote_both_proxy_and_producers, snax_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( double_register_unregister_proxy_keeps_votes, snax_system_tester ) try {
   //alice1111111 becomes a proxy
   BOOST_REQUIRE_EQUAL( success(), push_action( N(alice1111111), N(regproxy), mvo()
                                                ("proxy",  "alice1111111")
                                                ("isproxy",  1)
...
BOOST_FIXTURE_TEST_CASE( proxy_cannot_use_another_proxy, snax_system_tester ) try {
   //alice1111111 becomes a proxy
   BOOST_REQUIRE_EQUAL( success(), push_action( N(alice1111111), N(regproxy), mvo()
                                                ("proxy",  "alice1111111")
                                                ("isproxy",  1)
...
fc::mutable_variant_object config_to_variant( const snax::chain::chain_config& config ) {
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, snax_system_tester ) try {
   create_accounts_with_resources( {  N(defproducer1), N(defproducer2), N(defproducer3) } );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer1", 1) );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer2", 2) );
   BOOST_REQUIRE_EQUAL( success(), regproducer( "defproducer3", 3) );
...
BOOST_FIXTURE_TEST_CASE( buyname, snax_system_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(fail) }, N(dan) ), // dan shouldn't be able to create fail
                            snax_assert_message_exception, snax_assert_message_is( "no active bid for name" ) );
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(nofail) }, N(dan) ), // dan shoudn't be able to do this, sam won
                            snax_assert_message_exception, snax_assert_message_is( "only highest bidder can claim" ) );
   ...
   transfer( "snax", "nofail", core_from_string( "1000.0000" ) );
   ...
   BOOST_REQUIRE_EXCEPTION( create_accounts_with_resources( { N(test.fail) }, N(dan) ), // dan shouldn't be able to do this
                            snax_assert_message_exception, snax_assert_message_is( "only suffix may create this account" ) );

   create_accounts_with_resources( { N(goodgoodgood) }, N(dan) ); /// 12 char names should succeed
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( bid_invalid_names, snax_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, snax_system_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( setparams, snax_system_tester ) try {
   //install multisig contract
   abi_serializer msig_abi_ser = initialize_multisig();
   auto producer_names = active_and_vote_producers();
   ...
   snax::chain::chain_config params;

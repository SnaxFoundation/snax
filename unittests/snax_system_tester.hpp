...
#include <snax.msig/snax.msig.wast.hpp>
#include <snax.msig/snax.msig.abi.hpp>
...
namespace snax_system {

class snax_system_tester : public TESTER {
public:

   snax_system_tester()
   : snax_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   snax_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(snax.token), N(snax.ram), N(snax.ramfee), N(snax.stake),
               N(snax.bpay), N(snax.vpay), N(snax.transf), N(snax.names) });
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(snax.token) );
         abi_def abi;
      ...
      BOOST_REQUIRE_EQUAL( core_from_string(("100000000000.0000")), get_balance("snax")  + get_balance("snax.ramfee") + get_balance("snax.stake") + get_balance("snax.ram") );
   }
   ...
   transaction_trace_ptr setup_producer_accounts( const std::vector<account_name>& accounts ) {
      ...
      signed_transaction trx;
      set_transaction_headers(trx);
      asset cpu = core_from_string(("8000.0000"));
      asset net = core_from_string(("8000.0000"));
      asset ram = core_from_string(("100.0000"));
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& snaxin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",snaxin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(snax.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(snax.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(snax.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
                                ("quantity", amount)
                                ("memo", "")
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = snax::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(snax.token), symbol_code, N(stat), symbol_code );
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "snax_global_state", data, abi_serializer_max_time );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(snax.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "snax", "snax.msig", core_from_string(("500000.0000")) ) );
         produce_block();
         ...
         set_code( N(snax.msig), snax_msig_wast );
         set_abi( N(snax.msig), snax_msig_abi );

         produce_blocks();
         const auto& accnt = control->db().get<account_object,by_name>( N(snax.msig) );
         abi_def msig_abi;
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "snax", "alice1111111", core_from_string(("65000000000.0000")), "snax" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::snax_code_name}, 1},
      ...
      //vote for producers
      {
         transfer( config::system_account_name, "alice1111111", core_from_string(("10000000000.0000")), config::system_account_name );
         BOOST_REQUIRE_EQUAL(success(), stake( "alice1111111", core_from_string(("3000000000.0000")), core_from_string(("3000000000.0000")) ) );
         BOOST_REQUIRE_EQUAL(success(), buyram( "alice1111111", "alice1111111", core_from_string(("3000000000.0000")) ) );
   ...
   void cross_15_percent_threshold() {
      ...
      regproducer(N(producer1111));
      {
         ...
         trx.actions.emplace_back( get_action( config::system_account_name, N(delegatebw),
                                               ...
                                               ("stake_net_quantity", core_from_string(("15000000000.0000")) )
                                               ("stake_cpu_quantity", core_from_string(("000.0000")) )
                                               ("transfer", 1 )
                                             )
                                 );
         ...
         trx.actions.emplace_back( get_action( config::system_account_name, N(undelegatebw),
                                               ...
                                               ("from", "producer1111")
                                               ("receiver", "producer1111")
                                               ("unstake_net_quantity", core_from_string(("15000000000.0000")) )
                                               ("unstake_cpu_quantity", core_from_string(("000.0000")) )
                                             )
                                 );
...
inline uint64_t M( const string& snax_str ) {
   return core_from_string( snax_str ).get_amount();
}

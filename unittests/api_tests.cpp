...
#define DISABLE_SNAXLIB_SERIALIZE
...
bool is_access_violation(fc::unhandled_exception const & e) {
   try {
    ...
    catch (const snax::chain::wasm_execution_error& e) {
       return true;
    } catch (...) {
...
bool is_wasm_execution_error(snax::chain::wasm_execution_error const& e) {return true;}
...
BOOST_FIXTURE_TEST_CASE(action_receipt_tests, TESTER) { try {
   ...
   set_code( config::system_account_name, snax_bios_wast );

	set_code( N(testapi), snax_bios_wast );
   set_abi(N(testapi), snax_bios_abi);
	set_code( N(testapi), test_api_wast );
...
BOOST_FIXTURE_TEST_CASE(action_tests, TESTER) { try {
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "assert_false", {} ),
                          snax_assert_message_exception, snax_assert_message_is("test_action::assert_false") );
   ...
   BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION( *this, "test_action", "read_action_to_0", raw_bytes), snax::chain::wasm_execution_error,
         [](const snax::chain::wasm_execution_error& e) {
   ...
   // test read_action_to_64k
   raw_bytes.resize(3);
	BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION( *this, "test_action", "read_action_to_64k", raw_bytes ), snax::chain::wasm_execution_error,
         [](const snax::chain::wasm_execution_error& e) {
            return expect_assert_message(e, "access violation");
         }
      );
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "test_current_time", fc::raw::pack(now) ),
                          snax_assert_message_exception, snax_assert_message_is("tmp == current_time()")     );
...
BOOST_FIXTURE_TEST_CASE(cf_action_tests, TESTER) { try {
      ...
      BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_transaction", "send_cf_action_fail", {} ),
                             ...
                             snax_assert_message_is("context free actions cannot have authorizations") );
...
BOOST_FIXTURE_TEST_CASE(transaction_tests, TESTER) { try {
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION(*this, "test_transaction", "send_action_inline_fail", {}),
                          snax_assert_message_exception,
                          snax_assert_message_is("test_action::assert_false")                          );
   ...
   BOOST_CHECK_EXCEPTION(CALL_TEST_FUNCTION(*this, "test_transaction", "send_action_recurse", {}), snax::chain::transaction_exception,
         [](const snax::chain::transaction_exception& e) {
...
#warning re-enable deferred transaction replacement test after bug has been fixed
...

   {
      ...
      auth.accounts.push_back( permission_level_weight{{N(testapi), config::snax_code_name}, 1} );
      ...
      // But not anymore. Now it should subjectively fail because testapi@additional permission is not unilaterally satisfied by testapi@snax.code.
      dtt_action dtt_act3;
...
BOOST_FIXTURE_TEST_CASE(multi_index_tests, TESTER) { try {
   ...
   CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION( *this, "test_multi_index", "idx64_pk_iterator_exceed_end", {},
                                           snax_assert_message_exception, "cannot increment end iterator");
   ...
   CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION( *this, "test_multi_index", "idx64_pk_iterator_exceed_begin", {},
                                           snax_assert_message_exception, "cannot decrement iterator at beginning of table");
   CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION( *this, "test_multi_index", "idx64_sk_iterator_exceed_begin", {},
                                           snax_assert_message_exception, "cannot decrement iterator at beginning of index");
   CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION( *this, "test_multi_index", "idx64_pass_pk_ref_to_other_table", {},
                                           snax_assert_message_exception, "object passed to iterator_to is not in multi_index");
   ...
   CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION( *this, "test_multi_index", "idx64_pass_pk_end_itr_to_modify", {},
                                           snax_assert_message_exception, "cannot pass end iterator to modify");
   CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION( *this, "test_multi_index", "idx64_pass_pk_end_itr_to_erase", {},
                                           snax_assert_message_exception, "cannot pass end iterator to erase");
...
BOOST_FIXTURE_TEST_CASE(fixedpoint_tests, TESTER) { try {
	...
	CALL_TEST_FUNCTION_AND_CHECK_EXCEPTION( *this, "test_fixedpoint", "test_division_by_0", {},
                                          snax_assert_message_exception, "divide by zero"     );
...
BOOST_FIXTURE_TEST_CASE(permission_tests, TESTER) { try {
   ...
   CALL_TEST_FUNCTION( *this, "test_permission", "check_authorization",
      fc::raw::pack( check_auth {
         ...
         .permission = N(active),
         .pubkeys    = {
            public_key_type(string("SNAX7GfRtyDWWgxV88a5TRaYY59XmHptyfjsFmHHfioGNJtPjpSmGX"))
         }
      })
   );
...
/*************************************************************************************
 * snax_assert_code_tests test cases
 *************************************************************************************/
BOOST_FIXTURE_TEST_CASE(snax_assert_code_tests, TESTER) { try {
   produce_block();
   create_account( N(testapi) );
   produce_block();
   set_code(N(testapi), test_api_wast);
   ...
   const char* abi_string = R"=====(
{
   ...
   BOOST_CHECK_EXCEPTION( CALL_TEST_FUNCTION( *this, "test_action", "test_assert_code", fc::raw::pack((uint64_t)42) ),
                          snax_assert_code_exception, snax_assert_code_is(42)                                        );

...
#include <snax.sudo/snax.sudo.wast.hpp>
#include <snax.sudo/snax.sudo.abi.hpp>
...
class snax_sudo_tester : public tester {
public:

   snax_sudo_tester() {
      create_accounts( { N(snax.msig), N(prod1), N(prod2), N(prod3), N(prod4), N(prod5), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(snax.sudo), snax_sudo_wast, &system_private_key );
      set_abi( N(snax.sudo), snax_sudo_abi, &system_private_key );
      ...
      const auto& accnt = control->db().get<account_object,by_name>( N(snax.sudo) );
      abi_def abi;
      BOOST_REQUIRE_EQUAL(abi_serializer::to_abi(accnt.abi, abi), true);
      abi_ser.set_abi(abi, abi_serializer_max_time);

      while( control->pending_block_state()->header.producer.to_string() == "snax" ) {
         produce_block();
      }
   }

   void propose( name proposer, name proposal_name, vector<permission_level> requested_permissions, const transaction& trx ) {
      push_action( N(snax.msig), N(propose), proposer, mvo()
                     ("proposer",      proposer)
   ...
   void approve( name proposer, name proposal_name, name approver ) {
      push_action( N(snax.msig), N(approve), approver, mvo()
                     ("proposer",      proposer)
                     ("proposal_name", proposal_name)
   ...
   void unapprove( name proposer, name proposal_name, name unapprover ) {
      push_action( N(snax.msig), N(unapprove), unapprover, mvo()
                     ("proposer",      proposer)
                     ("proposal_name", proposal_name)
...
transaction snax_sudo_tester::sudo_exec( account_name executer, const transaction& trx, uint32_t expiration ) {
   fc::variants v;
...
transaction snax_sudo_tester::reqauth( account_name from, const vector<permission_level>& auths, uint32_t expiration ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(snax_sudo_tests)

BOOST_FIXTURE_TEST_CASE( sudo_exec_direct, snax_sudo_tester ) try {
...

   {
      ...
      sudo_trx.actions.emplace_back( get_action( N(snax.sudo), N(exec),
                                                 {{N(alice), config::active_name}, {N(snax.sudo), config::active_name}},
                                                 mvo()
                                                   ("executer", "alice")
                                                   ("trx", trx)
      ) );
   ...
   BOOST_REQUIRE_EQUAL( "snax", name{trace->action_traces[0].act.account} );
   BOOST_REQUIRE_EQUAL( "reqauth", name{trace->action_traces[0].act.name} );
   BOOST_REQUIRE_EQUAL( transaction_receipt::executed, trace->receipt->status );

} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( sudo_with_msig, snax_sudo_tester ) try {
   auto trx = reqauth( N(bob), {permission_level{N(bob), config::active_name}} );
   ...
   push_action( N(snax.msig), N(exec), N(alice), mvo()
                  ("proposer",      "carol")
                  ("proposal_name", "first")
                  ("executer",      "alice")
   );
   ...
   BOOST_REQUIRE_EQUAL( "snax.sudo", name{traces[0]->action_traces[0].act.account} );
   ...
   BOOST_REQUIRE_EQUAL( "snax", name{traces[1]->action_traces[0].act.account} );
...
BOOST_FIXTURE_TEST_CASE( sudo_with_msig_unapprove, snax_sudo_tester ) try {
   auto trx = reqauth( N(bob), {permission_level{N(bob), config::active_name}} );
   auto sudo_trx = sudo_exec( N(alice), trx );
   ...
   // The proposal should not have sufficient approvals to pass the authorization checks of snax.sudo::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(snax.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
                                       ), snax_assert_message_exception,
                                          snax_assert_message_is("transaction authorization failed")
   );

} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( sudo_with_msig_producers_change, snax_sudo_tester ) try {
   ...
   // The proposal has four of the five requested approvals but they are not sufficient to satisfy the authorization checks of snax.sudo::exec.
   BOOST_REQUIRE_EXCEPTION( push_action( N(snax.msig), N(exec), N(alice), mvo()
                                          ("proposer",      "carol")
                                          ("proposal_name", "first")
                                          ("executer",      "alice")
   ...
   BOOST_REQUIRE_EXCEPTION( approve( N(carol), N(first), N(newprod1) ),
                            snax_assert_message_exception,
                            snax_assert_message_is("approval is not on the list of requested approvals")
   );

...
class snax_msig_tester : public tester {
public:

   snax_msig_tester() {
      create_accounts( { N(snax.msig), N(snax.stake), N(snax.ram), N(snax.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(snax.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(snax.msig), name, accounts, data );
      produce_block();
...
transaction snax_msig_tester::reqauth( account_name from, const vector<permission_level>& auths, const fc::microseconds& max_serialization_time ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(snax_msig_tests)

BOOST_FIXTURE_TEST_CASE( propose_approve_execute, snax_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_approve_unapprove, snax_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_by_two, snax_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_with_wrong_requested_auth, snax_msig_tester ) try {
   auto trx = reqauth("alice", vector<permission_level>{ { N(alice), config::active_name },  { N(bob), config::active_name } }, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( big_transaction, snax_msig_tester ) try {
   vector<permission_level> perm = { { N(alice), config::active_name }, { N(bob), config::active_name } };
   auto wasm = wast_to_wasm( snax_token_wast );
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, snax_msig_tester ) try {

   // required to set up the link between (snax active) and (snax.prods active)
   //
   //                  snax active
   //                       |
   //             snax.prods active (2/3 threshold)
   ...
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("snax", "active").get_public_key(), 1}},
   ...
   // execute by alice to replace the snax system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect([&]( const transaction_trace_ptr& t) { if (t->scheduled) { trace = t; } } );
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, snax_msig_tester ) try {

   // set up the link between (snax active) and (snax.prods active)
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("snax", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{config::producers_account_name, config::active_name}, 1}}), "owner",
      { { config::system_account_name, "active" } }, { get_private_key( config::system_account_name, "active" ) });
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      snax_assert_message_exception, snax_assert_message_is("transaction authorization failed")
   );

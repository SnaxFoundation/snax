...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(snax.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        create_accounts({N(snax.msig), N(snax.token), N(snax.ram), N(snax.ramfee), N(snax.stake), N(snax.transf), N(snax.vpay), N(snax.creator), N(snax.airdrop), N(snax.team), N(snax.bpay) });

        // Set code for the following accounts:
        //  - snax (code: snax.bios) (already set by tester constructor)
        //  - snax.msig (code: snax.msig)
        //  - snax.token (code: snax.token)
        set_code_abi(N(snax.msig), snax_msig_wast, snax_msig_abi);//, &snax_active_pk);
        set_code_abi(N(snax.token), snax_token_wast, snax_token_abi); //, &snax_active_pk);

        // Set privileged for snax.msig and snax.token
        set_privileged(N(snax.msig));
        set_privileged(N(snax.token));

        // Verify snax.msig and snax.token is privileged
        const auto& snax_msig_acc = get<account_object, by_name>(N(snax.msig));
        BOOST_TEST(snax_msig_acc.privileged == true);
        const auto& snax_token_acc = get<account_object, by_name>(N(snax.token));
        BOOST_TEST(snax_token_acc.privileged == true);


        // Create SNAX tokens in snax.token, set its manager as snax
        auto max_supply = core_from_string(("10000000000000.0000")); /// 1x larger than 1B initial tokens
        auto initial_supply = core_from_string(("1000000000000.0000")); /// 1x larger than 1B initial tokens
        create_currency(N(snax.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SNAX tokens to test_1_snax.system
        issue(N(snax.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        // Set test_1_snax.system to snax
        set_code_abi(config::system_account_name, test_1_snax_system_wast, test_1_snax_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(snax.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "snax");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), snax_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string(("4999950000.0000")), core_from_string(("4999950000.0000"))), snax_assert_message_exception);
        ...
        undelegate_bandwidth(N(b1), N(b1), core_from_string(("4999950000.0000")), core_from_string(("4999950000.0000")));

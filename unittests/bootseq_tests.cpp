...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        create_accounts({N(snax.msig), N(snax.token), N(snax.ram), N(snax.ramfee), N(snax.stake), N(snax.vpay), N(snax.creator), N(snax.airdrop), N(snax.team), N(snax.bpay), N(snax.saving), N(snax.util) });

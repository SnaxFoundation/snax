...
def monitor_exchange():
    ...
    results = clisnax('get actions tokenxchange {} 0 -j'.format(action_num))
...
def transfer(to, quantity):
    if quantity[:-4] != ' SNAX':
        quantity += ' SNAX'
    results = clisnax('transfer tokenxchange {} "{}" {} -j'.format(to, quantity, KEY_TO_INTERNAL_ACCOUNT))

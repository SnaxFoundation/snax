#!/bin/bash
#
# snax-tn_up is a helper script used to start a node that was previously stopped.
# It is not intended to be run stand-alone; it is a companion to the
# snax-tn_bounce.sh and snax-tn_roll.sh scripts.

connected="0"

rundir=programs/snaxnode
prog=snaxnode

# Quote any args that are "*", so they are not expanded
qargs=`echo "$*" | sed -e 's/ \* / "*" /' -e 's/ \*$/ "*"/'`

if [ "$PWD" != "$SNAX_HOME" ]; then
    echo $0 must only be run from $SNAX_HOME
    exit -1
fi
...
datadir=var/lib/node_$SNAX_NODE
...
relaunch() {
    echo "$rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/snax/node_$SNAX_NODE > $datadir/stdout.txt  2>> $datadir/$log "
    nohup $rundir/$prog $qargs $* --data-dir $datadir --config-dir etc/snax/node_$SNAX_NODE > $datadir/stdout.txt  2>> $datadir/$log &
    pid=$!
    echo pid = $pid

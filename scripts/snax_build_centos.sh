	...
	if [ "${OS_VER}" -lt 7 ]; then
		printf "\\n\\tYou must be running Centos 7 or higher to install SNAX.\\n"
		printf "\\tExiting now.\\n\\n"
		exit 1;
	fi

	if [ "${DISK_AVAIL%.*}" -lt "${DISK_MIN}" ]; then
		printf "\\n\\tYou must have at least %sGB of available storage to install SNAX.\\n" "${DISK_MIN}"
		printf "\\tExiting now.\\n\\n"
		exit 1;
	fi
	...
	printf "\\n"
        ...
        if ! YUM=$( command -v yum 2>/dev/null ); then
                printf "\\t!! Yum must be installed to compile SNAX !!\\n"
                printf "\\tExiting now.\\n"
                exit 1;
        fi
	...
	if [ "${COUNT}" -gt 1 ]; then
		printf "\\tThe following dependencies are required to install SNAX.\\n"
		printf "\\t${DISPLAY}\\n\\n"

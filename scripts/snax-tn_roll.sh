#!/bin/bash
#
# snax-tn_roll is used to have all of the instances of the SNAX daemon on a host brought down
# so that the underlying executable image file (the "text file") can be replaced. Then
# all instances are restarted.
# usage: snax-tn_roll.sh [arglist]
...
# the command line.
#
# In most cases, simply running ./snax-tn_roll.sh is sufficient.
#

if [ -z "$SNAX_HOME" ]; then
    echo SNAX_HOME not set - $0 unable to proceed.
    exit -1
fi
...
prog=""
RD=""
for p in snaxd snaxd snaxnode; do
    prog=$p
    RD=bin
    if [ -f $RD/$prog ]; then
        break;
...
if [ \( -z "$prog" \) -o \( -z "$RD" \) ]; then
    echo unable to locate binary for snaxd or snaxd or snaxnode
    exit 1
fi
...
if [ $DD = "all" ]; then
    for SNAX_RESTART_DATA_DIR in `ls -d var/lib/node_??`; do
        bash $SNAX_HOME/scripts/snax-tn_up.sh "$*"
    done
else
    bash $SNAX_HOME/scripts/snax-tn_up.sh "$*"
fi
unset SNAX_RESTART_DATA_DIR

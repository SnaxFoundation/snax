...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.snaxAccount])
        ...
        for fromIndex in range(namedAccounts.numAccounts):
            ...
            try:
                ...
                if trans is None or not trans[0]:
                    timeOutCount+=1
                    if timeOutCount>=3:
                       Print("Failed to push create action to snax contract for %d consecutive times, looks like snaxnode already exited." % (timeOutCount))
                       keepProcessing=False
                       break
                    Print("Failed to push create action to snax contract. sleep for 60 seconds")
                    time.sleep(60)
                else:
                    timeOutCount=0
                time.sleep(1)

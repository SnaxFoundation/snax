...
class Cluster(object):
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraSnaxnodeArgs=None, useBiosBootFile=True, specificExtraSnaxnodeArgs=None):
        """Launch cluster.
        ...
        extraSnaxnodeArgs: string of arguments to pass through to each nodoes instance (via --snaxnode flag on launcher)
        ...
        if extraSnaxnodeArgs is not None:
            assert(isinstance(extraSnaxnodeArgs, str))
            snaxnodeArgs += extraSnaxnodeArgs
        if Utils.Debug:

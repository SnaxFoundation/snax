...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="snax-ignition-wd/bootlog.txt"
    __configDir="etc/snax/"
    __dataDir="var/lib/"
    __fileDivider="================================================================="

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is kxd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="SNAXtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kxd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: snax server host
        port: snax server port
        walletHost: snax wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures snax mongo plugin
        ...
        self.snaxAccount=self.defProducerAccounts["snax"]= Account("snax")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, extraNodsnaxArgs=None, useBiosBootFile=True, specificExtraSnaxnodeArgs=None):
        """Launch cluster.
        ...
        extraNodsnaxArgs: string of arguments to pass through to each nodoes instance (via --snaxnode flag on launcher)
        ...
        specificExtraSnaxnodeArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-snaxnode flags on launcher), example: { "5" : "--plugin snax::test_control_api_plugin" }
        """
        assert(isinstance(topo, str))

        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.SnaxServerName))
            return True
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on snaxnode default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
        ...
        cmd="%s -p %s -n %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.SnaxLauncherPath, pnodes, totalNodes, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            p2pPlugin, producerFlag)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        snaxnodeArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            snaxnodeArgs += " --plugin snax::wallet_api_plugin"
        if self.enableMongo:
            snaxnodeArgs += " --plugin snax::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodsnaxArgs is not None:
            assert(isinstance(extraNodsnaxArgs, str))
            snaxnodeArgs += extraNodsnaxArgs
        if Utils.Debug:
            snaxnodeArgs += " --contracts-console"

        if snaxnodeArgs:
            cmdArr.append("--snaxnode")
            cmdArr.append(snaxnodeArgs)

        if specificExtraSnaxnodeArgs is not None:
            assert(isinstance(specificExtraSnaxnodeArgs, dict))
            for nodeNum,arg in specificExtraSnaxnodeArgs.items():
                assert(isinstance(nodeNum, (str,int)))
                assert(isinstance(arg, str))
                cmdArr.append("--specific-num")
                cmdArr.append(str(nodeNum))
                cmdArr.append("--specific-snaxnode")
                cmdArr.append(arg)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.SnaxServerName, totalNodes, len(nodes)))
            return False
        ...
        self.snaxAccount=self.defProducerAccounts["snax"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.SnaxClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.SnaxClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on snax server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextSnaxIdx: %d, n: %d" % (nextSnaxIdx, n))
                nextSnaxIdx=(nextSnaxIdx + 1)%count
                if not self.nodes[nextSnaxIdx].killed:
                    #Utils.Print("nextSnaxIdx: %d" % (nextSnaxIdx))
            ...
            #Utils.Print("nextSnaxIdx: %d, count: %d" % (nextSnaxIdx, count))
            node=self.nodes[nextSnaxIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.SnaxServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on snax node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        receiving transferAmount*n SNAX and forwarding x-transferAmount funds. Transfer actions are spread round-robin across the cluster to vaidate system cohesiveness."""

        if Utils.Debug: Utils.Print("Get initial system balances.")
        initialBalances=self.nodes[0].getSnaxBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.snaxAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def createAccountAndVerify(self, account, creator, stakedDeposit=1000, stakeNet=100, stakeCPU=100, buyram=1000000):
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse snax private keys from cluster config files.")
            return None
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        """Bootstrap cluster using the bios_boot.sh script generated by snax-launcher."""
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down snax cluster.")
            return None
        ...
        snaxName="snax"
        snaxKeys=producerKeys[snaxName]
        snaxAccount=Account(snaxName)
        snaxAccount.ownerPrivateKey=snaxKeys["private"]
        snaxAccount.ownerPublicKey=snaxKeys["public"]
        snaxAccount.activePrivateKey=snaxKeys["private"]
        snaxAccount.activePublicKey=snaxKeys["public"]
        producerKeys.pop(snaxName)

        if not walletMgr.importKey(snaxAccount, ignWallet):
            Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (snaxName))
            return None
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"snax\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init transfer")
            opts="--permission snax@active"
            if name != "snax":
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from snax.token to %s." % (name))
                    return None
    ...
    def bootstrap(totalNodes, prodCount, totalProducers, biosHost, biosPort, walletMgr, onlyBios=False):
        """Create 'prodCount' init accounts and deposits 10000000000 SNAX in each. If prodCount is -1 will initialize all possible producers.
        ...
        trans=biosNode.publishContract(snaxAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
        ...
        for name, keys in producerKeys.items():
            ...
            trans=biosNode.createAccount(initx, snaxAccount, 0)
            if trans is None:
        ...
        if not onlyBios:
            if prodCount == -1:
                ...
                with open(setProdsFile, "r") as f:
                    ...
                    opts="--permission snax@active"
                    myTrans=biosNode.pushMessage("snax", "setprods", setProdsStr, opts)
                ...
                trans=biosNode.pushMessage("snax", "setprods", setProdsStr, opts)
                if trans is None or not trans[0]:
            ...
            # wait for block production handover (essentially a block produced by anyone but snax).
            lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "snax"
            ret=Utils.waitForBool(lam)
            if not ret:
                Utils.Print("ERROR: Block production handover failed.")
                return None

        snaxTokenAccount=copy.deepcopy(snaxAccount)
        snaxTokenAccount.name="snax.token"
        trans=biosNode.createAccount(snaxTokenAccount, snaxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (snaxTokenAccount.name))
            return None

        snaxRamAccount=copy.deepcopy(snaxAccount)
        snaxRamAccount.name="snax.ram"
        trans=biosNode.createAccount(snaxRamAccount, snaxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (snaxRamAccount.name))
            return None

        snaxRamfeeAccount=copy.deepcopy(snaxAccount)
        snaxRamfeeAccount.name="snax.ramfee"
        trans=biosNode.createAccount(snaxRamfeeAccount, snaxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (snaxRamfeeAccount.name))
            return None

        snaxStakeAccount=copy.deepcopy(snaxAccount)
        snaxStakeAccount.name="snax.stake"
        trans=biosNode.createAccount(snaxStakeAccount, snaxAccount, 0)
        if trans is None:
            Utils.Print("ERROR: Failed to create account %s" % (snaxStakeAccount.name))
            return None
        ...
        abiFile="%s.abi" % (contract)
        Utils.Print("Publish %s contract" % (contract))
        trans=biosNode.publishContract(snaxTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
        if trans is None:
            Utils.Print("ERROR: Failed to publish contract %s." % (contract))
            return None

        # Create currency0000, followed by issue currency0000
        contract=snaxTokenAccount.name
        Utils.Print("push create action to %s contract" % (contract))
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (snaxTokenAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push create action to snax contract.")
            return None
        ...
        action="issue"
        data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (snaxAccount.name, CORE_SYMBOL)
        opts="--permission %s@active" % (contract)
        trans=biosNode.pushMessage(contract, action, data, opts)
        if trans is None or not trans[0]:
            Utils.Print("ERROR: Failed to push issue action to snax contract.")
            return None
        ...
        Utils.Print("Verify snax issue, Expected: %s" % (expectedAmount))
        actualAmount=biosNode.getAccountSnaxBalanceStr(snaxAccount.name)
        ...
        action="transfer"
        for name, keys in producerKeys.items():
            data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (snaxAccount.name, name, initialFunds, "init transfer")
            opts="--permission %s@active" % (snaxAccount.name)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (snaxTokenAccount.name, name))
                return None
    ...
    @staticmethod
    def pgrepSnaxServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.SnaxServerName)
    ...
    @staticmethod
    def pgrepSnaxServerPattern(nodeInstance):
        dataLocation=Cluster.__dataDir + Cluster.nodeExtensionToName(nodeInstance)
        return r"[\n]?(\d+) (.* --data-dir %s .*)\n" % (dataLocation)

    # Populates list of SnaxInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=None):
        nodes=[]

        psOut=Cluster.pgrepSnaxServers(timeout)
        if psOut is None:
        ...
        for i in range(0, totalNodes):
            pattern=Cluster.pgrepSnaxServerPattern(i)
            m=re.search(pattern, psOut, re.MULTILINE)
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.SnaxServerName, pattern))
                break
    ...
    def discoverBiosNodePid(self, timeout=None):
        psOut=Cluster.pgrepSnaxServers(timeout=timeout)
        pattern=Cluster.pgrepSnaxServerPattern("bios")
        Utils.Print("pattern={\n%s\n}, psOut=\n%s\n" % (pattern,psOut))
        m=re.search(pattern, psOut, re.MULTILINE)
        if m is None:
            Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.SnaxServerName, pattern))
    ...
    # Kills a percentange of Snax instances starting from the tail and update snaxInstanceInfos state
    def killSomeSnaxInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.SnaxServerName, killSignal))
    ...
    def relaunchSnaxInstances(self):
    ...
    def killall(self, silent=True, allInstances=False):
        """Kill cluster snaxnode instances. allInstances will kill all snaxnode instances running on the system."""
        cmd="%s -k 9" % (Utils.SnaxLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down snax cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the snax server
            cmd="pkill -9 %s" % (Utils.SnaxServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down snax cluster.")
    ...
    def bounce(self, nodes, silent=True):
        """Bounces snaxnode instances as indicated by parameter nodes.
    ...
    def down(self, nodes, silent=True):
        """Brings down snaxnode instances as indicated by parameter nodes.
        nodes should take the form of a comma-separated list as accepted by the launcher --bounce command (e.g. '00' or '00,01')"""
        cmdArr = Cluster.__LauncherCmdArr.copy()
        cmdArr.append("--down")
        cmdArr.append(nodes)

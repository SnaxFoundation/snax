...
WalletdName=Utils.SnaxWalletName
ClientName="clisnax"
...
try:
    ...
    if localTest and not dontLaunch:
        ...
        if walletMgr.launch() is False:
            ...
            errorExit("Failed to stand up snax walletd.")
    ...
    if not dontLaunch:
        walletAccounts.append(cluster.snaxAccount)
    ...
    transId=node.createInitializeAccount(currencyAccount, cluster.defproduceraAccount, buyRAM=400000, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.defproduceraAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.defproduceraAccount, buyRAM=400000, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountSnaxBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountSnaxBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    contractDir="contracts/snax.token"
    wasmFile="snax.token.wasm"
    abiFile="snax.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getSnaxAccountFromDb(currencyAccount.name)
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get SNAX account failed", raw=True)
    ...
    account=node.getSnaxAccount(defproduceraAccount.name, exitOnError=True)
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killSnaxInstances, killWallet, keepLogs, killAll, dumpErrorDetails)

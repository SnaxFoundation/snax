...
killAll()
{
  programs/snax-launcher/snax-launcher -k 15
}

cleanup()
{
    rm -rf etc/snax/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clisnax/clisnax get info)"
  verifyErrorCode "clisnax get info"
...
# stand up snaxnode cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --snaxnode \"--plugin snax::wallet_api_plugin\" $launcherOpts
programs/snax-launcher/snax-launcher --snaxnode "--plugin snax::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clisnax/clisnax --port $port2 get block 1)"
    verifyErrorCode "clisnax get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clisnax/clisnax create key)"
verifyErrorCode "clisnax create key"
...
PASSWORD_INITA="$(programs/clisnax/clisnax wallet create --name inita)"
verifyErrorCode "clisnax wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clisnax/clisnax wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clisnax wallet import"
programs/clisnax/clisnax wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clisnax wallet import"
programs/clisnax/clisnax wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clisnax wallet import"
programs/clisnax/clisnax wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clisnax wallet import"
...
ACCOUNT_INFO="$(programs/clisnax/clisnax create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clisnax create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clisnax/clisnax get account testera)"
verifyErrorCode "clisnax get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clisnax/clisnax transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clisnax transfer"
   ...
   TRANS_INFO="$(programs/clisnax/clisnax --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clisnax get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done

...
###############################################################
# Test for validating the dirty db flag sticks repeated snaxnode restart attempts
###############################################################
...
def runSnaxnodeAndGetOutput(myTimeout=3):
    """Startup snaxnode, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching snaxnode process.")
    cmd="programs/snaxnode/snaxnode --config-dir etc/snax/node_bios --data-dir var/lib/node_bios --verbose-http-errors --http-validate-host=false"
    Print("cmd: %s" % (cmd))
    proc=subprocess.Popen(cmd.split(), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if debug: Print("Snaxnode process launched.")

    output={}
    try:
        if debug: Print("Setting snaxnode process timeout.")
        outs,errs = proc.communicate(timeout=myTimeout)
        if debug: Print("Snaxnode process has exited.")
        output["stdout"] = outs.decode("utf-8")
        output["stderr"] = errs.decode("utf-8")
        output["returncode"] = proc.returncode
    except (subprocess.TimeoutExpired) as _:
        Print("ERROR: Snaxnode is running beyond the defined wait time. Hard killing snaxnode instance.")
...
try:
    ...
    for i in range(1,4):
        Print("Attempt %d." % (i))
        ret = runSnaxnodeAndGetOutput(timeout)
        assert(ret)
        assert(isinstance(ret, tuple))
        if not ret[0]:
            errorExit("Failed to startup snaxnode sucessfully on try number %d" % (i))
        assert(ret[1])
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killSnaxInstances, False, keepLogs, killAll, dumpErrorDetails)

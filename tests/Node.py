...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, walletMgr=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="SNAXtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def snaxClientArgs(self):
        walletArgs=" " + self.walletMgr.getWalletEndpointArgs() if self.walletMgr is not None else ""
        return self.endpointArgs + walletArgs + " " + Utils.MiscSnaxClientArgs
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getSnaxAccount(account.name, exitOnError=True)
            try:
    ...
    def getBlock(self, blockNum, silentErrors=False, exitOnError=False):
        ...
        if not self.enableMongo:
            ...
            return self.processClisnaxCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError, exitMsg=msg)
        else:
    ...
    def getTransaction(self, transId, silentErrors=False, exitOnError=False, delayedRetry=True):
        ...
        if not self.enableMongo:
            ...
            for i in range(0,(int(60/timeout) - 1)):
                trans=self.processClisnaxCmd(cmd, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnErrorForDelayed, exitMsg=msg)
    ...
    def getSnaxAccount(self, name, exitOnError=False):
        assert(isinstance(name, str))
        if not self.enableMongo:
            cmdDesc="get account"
            cmd="%s -j %s" % (cmdDesc, name)
            msg="( getSnaxAccount(name=%s) )" % (name);
            return self.processClisnaxCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
        else:
            return self.getSnaxAccountFromDb(name, exitOnError=exitOnError)

    def getSnaxAccountFromDb(self, name, exitOnError=False):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def getTable(self, contract, scope, table, exitOnError=False):
        ...
        return self.processClisnaxCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
    ...
    def getCurrencyBalance(self, contract, account, symbol=CORE_SYMBOL, exitOnError=False):
        ...
        return self.processClisnaxCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg, returnType=ReturnType.raw)
    ...
    def verifyAccount(self, account):
        assert(account)
        if not self.enableMongo:
            ret=self.getSnaxAccount(account.name)
            if ret is not None:
    ...
    def verifyAccountMdb(self, account):
        assert(account)
        ret=self.getSnaxAccountFromDb(account.name)
        if ret is not None:
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False, exitOnError=True):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.SnaxClientPath, self.snaxClientArgs(), source.name, destination.name)
        cmdArr=cmd.split()
    ...
    def currencyStrToInt(balanceStr):
        """Converts currency string of form "12.3456 SNAX" to int 123456"""
    ...
    def currencyIntToStr(balance, symbol):
        """Converts currency int of form 123456 to string "12.3456 SNAX" where SNAX is symbol string"""
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        """Validate each account has the expected SNAX balance. Validate cumulative balance matches expectedTotal."""
        assert(source)
        assert(isinstance(source, Account))
        assert(accounts)
        assert(isinstance(accounts, list))
        ...
        currentBalances=self.getSnaxBalances([source] + accounts)
    ...
    def getSnaxBalances(self, accounts):
        ...
        balances={}
        for account in accounts:
            balance = self.getAccountSnaxBalance(account.name)
    ...
    # Get actions mapped to an account (clisnax get actions)
    ...
    def getAccountSnaxBalanceStr(self, scope):
        """Returns SNAX currency0000 account balance from clisnax get table command. Returned balance is string following syntax "98.0311 SNAX". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("snax.token", scope)
        if Utils.Debug: Utils.Print("getNodeAccountSnaxBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountSnaxBalance(self, scope):
        """Returns SNAX currency0000 account balance from clisnax get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccountSnaxBalanceStr(scope)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd="%s %s push action -j %s %s" % (Utils.SnaxClientPath, self.snaxClientArgs(), account, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def setPermission(self, account, code, pType, requirement, waitForTransBlock=False, exitOnError=False):
        ...
        trans=self.processClisnaxCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError)
        self.trackCmdTransaction(trans)
    ...
    def delegatebw(self, fromAccount, netQuantity, cpuQuantity, toAccount=None, transferTo=False, waitForTransBlock=False, exitOnError=False):
        ...
        trans=self.processClisnaxCmd(cmd, cmdDesc, exitOnError=exitOnError, exitMsg=msg)
        self.trackCmdTransaction(trans)
    ...
    def processClisnaxCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        assert(isinstance(returnType, ReturnType))
        cmd="%s %s %s" % (Utils.SnaxClientPath, self.snaxClientArgs(), cmd)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
    ...
    def getInfo(self, silentErrors=False, exitOnError=False):
        cmdDesc = "get info"
        info=self.processClisnaxCmd(cmdDesc, cmdDesc, silentErrors=silentErrors, exitOnError=exitOnError)
        if info is None:
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by clisnax get info."""
        if not self.enableMongo:

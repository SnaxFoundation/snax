...
class TestHelper(object):
    ...
    def parse_args(includeArgs, applicationSpecificArgs=AppArgs()):
        ...
        if "--kill-count" in includeArgs:
            parser.add_argument("--kill-count", type=int, help="snaxnode instances to kill", default=-1)
        ...
        if "--host" in includeArgs:
            parser.add_argument("-h", "--host", type=str, help="%s host name" % (Utils.SnaxServerName),
                                     default=TestHelper.LOCAL_HOST)
        if "--port" in includeArgs:
            parser.add_argument("-p", "--port", type=int, help="%s host port" % Utils.SnaxServerName,
                                     default=TestHelper.DEFAULT_PORT)
        if "--wallet-host" in includeArgs:
            parser.add_argument("--wallet-host", type=str, help="%s host" % Utils.SnaxWalletName,
                                     default=TestHelper.LOCAL_HOST)
        if "--wallet-port" in includeArgs:
            parser.add_argument("--wallet-port", type=int, help="%s port" % Utils.SnaxWalletName,
        ...
        if "--dump-error-details" in includeArgs:
            parser.add_argument("--dump-error-details",
                                     help="Upon error print etc/snax/node_*/config.ini and var/lib/node_*/stderr.log to stdout",
                                     action='store_true')
        ...
        if "--clean-run" in includeArgs:
            parser.add_argument("--clean-run", help="Kill all snaxnode and klsnax instances", action='store_true')
        if "--sanity-test" in includeArgs:
            parser.add_argument("--sanity-test", help="Validates snaxnode and klsnax are in path and can be started up.", action='store_true')
    ...
    def printSystemInfo(prefix):
        ...
        Utils.Print("SNAX Client version: %s" % (clientVersion))
        Utils.Print("Processor: %s" % (platform.processor()))
        Utils.Print("OS name: %s" % (platform.platform()))
    
    @staticmethod
    # pylint: disable=too-many-arguments
    def shutdown(cluster, walletMgr, testSuccessful=True, killSnaxInstances=True, killWallet=True, keepLogs=False, cleanRun=True, dumpErrorDetails=False):
        ...
        assert(isinstance(killSnaxInstances, bool))

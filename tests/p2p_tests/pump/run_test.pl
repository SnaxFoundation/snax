...
my $snax_home = defined $ENV{SNAX_HOME} ? $ENV{SNAX_HOME} : getcwd;
my $snaxd = $snax_home . "/programs/snaxd/snaxd";
my $snaxc = $snax_home . "/programs/snaxc/snaxc";

my $nodes = defined $ENV{SNAX_TEST_RING} ? $ENV{SNAX_TEST_RING} : "1";
my $pnodes = defined $ENV{SNAX_TEST_PRODUCERS} ? $ENV{SNAX_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$snax_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of snaxd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"SNAX6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = snax::producer_plugin\n";
        print $cfg "plugin = snax::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$snaxc transfer snax inita 10 >> snaxc.out 2>> snaxc.err`;
            $counter++;
            if ($counter % 1000 == 0) {

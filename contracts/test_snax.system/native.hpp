...
#include <snaxlib/public_key.hpp>
#include <snaxlib/types.hpp>
#include <snaxlib/print.hpp>
#include <snaxlib/privileged.h>
#include <snaxlib/optional.hpp>
#include <snaxlib/producer_schedule.hpp>
...
namespace snaxsystem {
   ...
   struct authority {
      ...
      // explicit serialization macro is not necessary, used here only to improve compilation time
      SNAXLIB_SERIALIZE( authority, (threshold)(delay_sec)(keys)(accounts) )
   };
   ...
   struct block_header {
      ...
      snax::optional<snax::producer_schedule> new_producers;

      // explicit serialization macro is not necessary, used here only to improve compilation time
      SNAXLIB_SERIALIZE(block_header, (timestamp)(producer)(confirmed)(previous)(transaction_mroot)(action_mroot)
    ...
    */
   class native : public snax::contract {
      public:

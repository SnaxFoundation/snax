...
class simpletoken : public snax::contract {
   public:
      ...
      void transfer( account_name from, account_name to, uint64_t quantity ) {
         ...
         snax_assert( fromacnt.balance >= quantity, "overdrawn balance" );
   ...
   private:
      ...
      snax::multi_index<N(accounts), account> _accounts;
      ...
      void add_balance( account_name payer, account_name to, uint64_t q ) {
         ...
         if( toitr == _accounts.end() ) {
           ...
           _accounts.modify( toitr, 0, [&]( auto& a ) {
              a.balance += q;
              snax_assert( a.balance >= q, "overflow detected" );
           });
         }
      }
};

SNAX_ABI( simpletoken, (transfer)(issue) )

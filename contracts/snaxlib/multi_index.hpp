...
/**
 *  The indexed_by struct is used to instantiate the indices for the Multi-Index table. In SNAX, up to 16 secondary indices can be specified.
 *  @brief The indexed_by struct is used to instantiate the indices for the Multi-Index table. In SNAX, up to 16 secondary indices can be specified.
 *
 *  @tparam IndexName - is the name of the index. The name must be provided as an SNAX base32 encoded 64-bit integer and must conform to the SNAX naming requirements of a maximum of 13 characters, the first twelve from the lowercase characters a-z, digits 0-5, and ".", and if there is a 13th character, it is restricted to lowercase characters a-p and ".".
 *  @tparam Extractor - is a function call operator that takes a const reference to the table object type and returns either a secondary key type or a reference to a secondary key type. It is recommended to use the `snax::const_mem_fun` template, which is a type alias to the `boost::multi_index::const_mem_fun`. See the documentation for the Boost `const_mem_fun` key extractor for more details.
 *
 *  Example:
       *
*
 ...
 *  class mycontract: snax::contract {
 *    struct record {
 *       uint64_t    primary;
 *       uint128_t   secondary;
 *       uint64_t primary_key() const { return primary; }
 *       uint64_t get_secondary() const { return secondary; }
 *       SNAXLIB_SERIALIZE( record, (primary)(secondary) )
 *     };
 *    public:
 ...
 *  @brief Defines SNAX Multi Index Table
 *  @ingroup databasecpp
 *
 *
 *
 *  SNAX Multi-Index API provides a C++ interface to the SNAX database. It is patterned after Boost Multi Index Container.
 *  SNAX Multi-Index table requires exactly a uint64_t primary key. For the table to be able to retrieve the primary key,
 *  the object stored inside the table is required to have a const member function called primary_key() that returns uint64_t.
 *  SNAX Multi-Index table also supports up to 16 secondary indices. The type of the secondary indices could be any of:
 *  - uint64_t
 *  - uint128_t
 *  - uint256_t
 *  - double
 ...
 *      SNAXLIB_SERIALIZE( record, (primary)(secondary_1)(secondary_2)(secondary_3)(secondary_4)(secondary_5) )
 *    };
 *    public:
 *      mycontract( account_name self ):contract(self){}
 *      void myaction() {
...
class multi_index
{
   private:
      ...
      struct index {
         public:
            ...
            struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
               public:
                  ...
                  const_iterator& operator++() {
                     ...
                     snax_assert( _item != nullptr, "cannot increment end iterator" );
                  ...
                  const_iterator& operator--() {
                     ...
                     if( !_item ) {
                        ...
                        snax_assert( ei != -1, "cannot decrement end iterator when the index is empty" );
                        prev_itr = secondary_index_db_functions<secondary_key_type>::db_idx_previous( ei , &prev_pk );
                        snax_assert( prev_itr >= 0, "cannot decrement end iterator when the index is empty" );
                     } else {
                        if( _item->__iters[Number] == -1 ) {
                           secondary_key_type temp_secondary_key;
                           auto idxitr = secondary_index_db_functions<secondary_key_type>::db_idx_find_primary(_idx->get_code(), _idx->get_scope(), _idx->name(), _item->primary_key(), temp_secondary_key);
                        ...
                        snax_assert( prev_itr >= 0, "cannot decrement iterator at beginning of index" );
                     }
            ...
            const_iterator require_find( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               auto lb = lower_bound( secondary );
               snax_assert( lb != cend(), error_msg );
               snax_assert( secondary == secondary_extractor_type()(*lb), error_msg );
               return lb;
            }
            ...
            const T& get( const secondary_key_type& secondary, const char* error_msg = "unable to find secondary key" )const {
               ...
               snax_assert( result != cend(), error_msg );
               return *result;
            }
            ...
            const_iterator iterator_to( const T& obj ) {
               ...
               snax_assert( objitem.__idx == _multidx, "object passed to iterator_to is not in multi_index" );
            ...
            void modify( const_iterator itr, uint64_t payer, Lambda&& updater ) {
               snax_assert( itr != cend(), "cannot pass end iterator to modify" );

               _multidx->modify( *itr, payer, std::forward<Lambda&&>(updater) );
            }

            const_iterator erase( const_iterator itr ) {
               snax_assert( itr != cend(), "cannot pass end iterator to erase" );
      ...
      const item& load_object_by_primary_iterator( int32_t itr )const {
         ...
         snax_assert( size >= 0, "error reading iterator" );
   ...
   public:
       ...
       *
       *  Notes
       *  The `snax::multi_index` template has template parameters `<uint64_t TableName, typename T, typename... Indices>`, where:
       ...
       *  - It is recommended to use the snax::const_mem_fun template, which is a type alias to the boost::multi_index::const_mem_fun.  See the documentation for the Boost const_mem_fun key extractor for more details.
       *
       *  Example:
       *
       *  @code
       ...
       *       SNAXLIB_SERIALIZE( address, (account_name)(first_name)(last_name)(street)(city)(state) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef snax::multi_index< N(address), address > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self); // code, scope
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       */
       ...
       *        snax_assert(addresses.get_code() == N(dan), "Codes don't match.");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *      void myaction() {
       *        address_index addresses(N(dan), N(dan)); // code, scope
       *        snax_assert(addresses.get_code() == N(dan), "Scopes don't match");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      struct const_iterator : public std::iterator<std::bidirectional_iterator_tag, const T> {
         ...
         const_iterator& operator--() {
            ...
            if( !_item ) {
               ...
               snax_assert( ei != -1, "cannot decrement end iterator when the table is empty" );
               prev_itr = db_previous_i64( ei , &prev_pk );
               snax_assert( prev_itr >= 0, "cannot decrement end iterator when the table is empty" );
            } else {
               prev_itr = db_previous_i64( _item->__primary_itr, &prev_pk );
               snax_assert( prev_itr >= 0, "cannot decrement iterator at beginning of table" );
            }
       ...
       *          address.street = "1 SNAX Way";
       *          address.city = "Blacksburg";
       *          address.state = "VA";
       *        });
       *        auto itr = addresses.find(N(dan));
       *        snax_assert(itr == addresses.cbegin(), "Only address is not at front.");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        snax_assert(itr == addresses.begin(), "Only address is not at front.");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        snax_assert(itr != addresses.cend(), "Address for account doesn't exist");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        snax_assert(itr != addresses.end(), "Address for account doesn't exist");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        snax_assert(itr->account_name == N(dan), "Incorrect Last Record ");
       *        itr++;
       *        snax_assert(itr->account_name == N(brendan), "Incorrect Second Last Record");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        itr--;
       *        snax_assert(itr->account_name == N(brendan), "Incorrect First Record ");
       *        itr--;
       *        snax_assert(itr->account_name == N(dan), "Incorrect Second Record");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *       SNAXLIB_SERIALIZE( address, (account_name)(first_name)(last_name)(street)(city)(state)(zip) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef snax::multi_index< N(address), address, indexed_by< N(zip), const_mem_fun<address, uint64_t, &address::by_zip> > address_index;
       *      void myaction() {
       *        address_index addresses(_self, _self);  // code, scope
       *        // add to table, first argument is account to bill for storage
       *        addresses.emplace(payer, [&](auto& address) {
       ...
       *        snax_assert(itr->account_name == N(brendan), "Incorrect First Lower Bound Record ");
       *        itr++;
       *        snax_assert(itr->account_name == N(dan), "Incorrect Second Lower Bound Record");
       *        itr++;
       *        snax_assert(itr == zip_index.end(), "Incorrect End of Iterator");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        snax_assert(itr->account_name == N(dan), "Incorrect First Upper Bound Record ");
       *        itr++;
       *        snax_assert(itr == zip_index.end(), "Incorrect End of Iterator");
       *      }
       *  }
       ...
       *       SNAXLIB_SERIALIZE( address, (key)(first_name)(last_name)(street)(city)(state) )
       *    };
       *    public:
       *      addressbook(account_name self):contract(self) {}
       *      typedef snax::multi_index< N(address), address > address_index;
      ...
      uint64_t available_primary_key()const {
         ...
         snax_assert( _next_primary_key < no_available_primary_key, "next primary key in table is at autoincrement limit");
       ...
       *        snax_assert(itr->account_name == N(dan), "Incorrect Record ");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        snax_assert(iterator_to(user) == itr, "Invalid iterator");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      const_iterator iterator_to( const T& obj )const {
         const auto& objitem = static_cast<const item&>(obj);
         snax_assert( objitem.__idx == this, "object passed to iterator_to is not in multi_index" );
         return {this, &objitem};
      }
      /**
      ...
      const_iterator emplace( uint64_t payer, Lambda&& constructor ) {
         ...
         snax_assert( _code == current_receiver(), "cannot create objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        snax_assert(itr != addresses.end(), "Address for account not found");
      ...
      template<typename Lambda>
      void modify( const_iterator itr, uint64_t payer, Lambda&& updater ) {
         snax_assert( itr != end(), "cannot pass end iterator to modify" );
       ...
       *          address.state = "CA";
       *        });
       *        snax_assert(itr->city == "San Luis Obispo", "Address not modified");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      void modify( const T& obj, uint64_t payer, Lambda&& updater ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         snax_assert( objitem.__idx == this, "object passed to modify is not in multi_index" );
         auto& mutableitem = const_cast<item&>(objitem);
         snax_assert( _code == current_receiver(), "cannot modify objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         snax_assert( pk == obj.primary_key(), "updater cannot change primary key when modifying an object" );
       ...
       *        });
       *        auto user = addresses.get(N(dan));
       *        snax_assert(user.first_name == "Daniel", "Couldn't get him.");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        snax_assert(itr != addresses.end(), "Couldn't get him.");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       ...
       *        snax_assert(itr != addresses.end(), "Address for account not found");
       *        addresses.erase( itr );
       *        snax_assert(itr != addresses.end(), "Address not erased properly");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
       */
      const_iterator erase( const_iterator itr ) {
         snax_assert( itr != end(), "cannot pass end iterator to erase" );
       ...
       *        });
       *        auto itr = addresses.find(N(dan));
       *        snax_assert(itr != addresses.end(), "Record is not found");
       *        addresses.erase(*itr);
       *        itr = addresses.find(N(dan));
       *        snax_assert(itr == addresses.end(), "Record is not deleted");
       *      }
       *  }
       *  SNAX_ABI( addressbook, (myaction) )
       *  @endcode
      ...
      void erase( const T& obj ) {
         ...
         const auto& objitem = static_cast<const item&>(obj);
         snax_assert( objitem.__idx == this, "object passed to erase is not in multi_index" );
         snax_assert( _code == current_receiver(), "cannot erase objects in table of another contract" ); // Quick fix for mutating db using multi_index that shouldn't allow mutation. Real fix can come in RC2.
         ...
         snax_assert( itr2 != _items_vector.rend(), "attempt to remove object that was not in multi_index" );

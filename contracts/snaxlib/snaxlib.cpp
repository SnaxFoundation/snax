...
namespace snax {
   ...
   private:
      ...
      public:
         ...
         {
            snax_assert(_offset == _heap_size, "malloc_from_freed was designed to only be called after _heap was completely allocated");
...
{
   return snax::memory_heap.malloc(size);
}

void* calloc(size_t count, size_t size)
{
   void* ptr = snax::memory_heap.malloc(count*size);
...
{
   return snax::memory_heap.realloc(ptr, size);
}

void free(void* ptr)
{
   return snax::memory_heap.free(ptr);
}

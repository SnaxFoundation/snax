...
#define N(X) ::snax::string_to_name(#X)
namespace snax {
...
// Helper macro for SNAX_API
#define SNAX_API_CALL( r, OP, elem ) \
   case ::snax::string_to_name( BOOST_PP_STRINGIZE(elem) ): \
      snax::execute_action( &thiscontract, &OP::elem ); \
      break;

// Helper macro for SNAX_ABI
#define SNAX_API( TYPE,  MEMBERS ) \
   BOOST_PP_SEQ_FOR_EACH( SNAX_API_CALL, TYPE, MEMBERS )
 ...
 * To be able to use this macro, the contract needs to be derived from snax::contract
 * 
 ...
 * Example:
 * @code
 * SNAX_ABI( snax::bios, (setpriv)(setalimits)(setglimits)(setprods)(reqauth) )
 * @endcode
 */
#define SNAX_ABI( TYPE, MEMBERS ) \
extern "C" { \
   void apply( uint64_t receiver, uint64_t code, uint64_t action ) { \
      auto self = receiver; \
      if( action == N(onerror)) { \
         /* onerror is only valid if it is for the "snax" code account and authorized by "snax"'s "active permission */ \
         snax_assert(code == N(snax), "onerror action's are only valid from the \"snax\" system account"); \
      } \
      if( code == self || action == N(onerror) ) { \
         TYPE thiscontract( self ); \
         switch( action ) { \
            SNAX_API( TYPE, MEMBERS ) \
         } \

...
## snax.system::cfgproducer    account config
...
## snax.system::okproducer     account producer vote
...
## snax.system::setproxy       account proxy
...
## snax.system::unstake        account quantity 
...
## snax.system::withdraw account
   - this action can only be triggered by snax.system via a deferred transaction generated by unstake
   - this will generate an inline snax.token::transfer call from=snax.system to=account and amount equal to the next withdraw increment
   - this will generate another deferred withdraw to continue the process if there are still withdraws to be made


## snax.system::transfer from to amount memo
   - decrements balance of from if amount <= balance
   - increments balance of to by amount
   - memo is ignored 

## snax.system::stakevote account amount
   - the primary currency of snax is controlled by the token contract which enables users to transfer
...
## snax.system::onblock   account blocktime blocknum
...
## snax.system::freeze producer accounts true|false 
...
## snax.system::paystandby producer

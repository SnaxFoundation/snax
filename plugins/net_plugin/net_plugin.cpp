...
#include <snax/net_plugin/protocol.hpp>
...
using namespace snax::chain::plugin_interface::compat;
...
namespace snax {
   ...
   using snax::chain::transaction_id_type;
...
#define peer_elog( PEER, FORMAT, ... ) \
  ...
  FC_MULTILINE_MACRO_END
   ...
   struct update_request_time {
      ...
      void operator () (struct snax::peer_block_state &bs) {
         bs.requested_time = time_point::now();
      }
   } set_request_time;

   typedef multi_index_container<
      snax::peer_block_state,
      indexed_by<
         ordered_unique< tag<by_id>, member<snax::peer_block_state, block_id_type, &snax::peer_block_state::id > >,
         ordered_unique< tag<by_block_num>, member<snax::peer_block_state, uint32_t, &snax::peer_block_state::block_num > >
         >
      > peer_block_state_index;


   struct update_known_by_peer {
      void operator() (snax::peer_block_state& bs) {
         bs.is_known = true;
      }
      void operator() (transaction_state& ts) {
   ...
   public:
      ...
      void fill_out_buffer( std::vector<boost::asio::const_buffer>& bufs ) {
         if( _sync_write_queue.size() > 0 ) { // always send msgs from sync_write_queue first
            ...
            SNAX_ASSERT( _write_queue_size == 0, plugin_exception, "write queue size expected to be zero" );
         }
      }
   ...
   void net_plugin_impl::handle_message( connection_ptr c, const packed_transaction &msg) {
      ...
      controller& cc = my_impl->chain_plug->chain();
      if( cc.get_read_mode() == snax::db_read_mode::READ_ONLY ) {
...
//      dispatcher->bcast_transaction(md->packed_trx);
   ...
   void net_plugin::set_program_options( options_description& /*cli*/, options_description& cfg )
   {
      cfg.add_options()
         ...
         ( "agent-name", bpo::value<string>()->default_value("\"SNAX Test Agent\""), "The name supplied to identify this node amongst the peers.")
   ...
   void net_plugin::plugin_initialize( const variables_map& options ) {
      ...
      try {
         ...
         if( my->allowed_connections & net_plugin_impl::Specified )
            SNAX_ASSERT( options.count( "peer-key" ),

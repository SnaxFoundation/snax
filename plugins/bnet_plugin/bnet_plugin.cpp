...
#include <snax/bnet_plugin/bnet_plugin.hpp>
...
using snax::public_key_type;
using snax::chain_id_type;
using snax::block_id_type;
using snax::block_timestamp_type;
using std::string;
using snax::sha256;
using snax::signed_block_ptr;
using snax::packed_transaction_ptr;
using std::vector;
...
namespace snax {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              SNAX_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              SNAX_THROW(transaction_exception, "bad transaction");
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           SNAX_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         SNAX_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });

   ...
   try {
      ...
      if (options.count( "snapshot" )) {
         ...
         if( options.count( "genesis-json" )) {
            SNAX_ASSERT( !fc::exists( my->blocks_dir / "blocks.log" ),
                        plugin_config_exception,
...
read_only::get_info_results read_only::get_info(const read_only::get_info_params&) const {
   ...
   return {
      snax::utilities::common::itoh(static_cast<uint32_t>(app().version())),
      db.get_chain_id(),
   ...
   try {
      ...
      try {
         ...
         try {
            ...
            try {
               value = ( snax::chain::string_to_symbol( 0, str.c_str() ) >> 8 );
            } catch( ... ) {
